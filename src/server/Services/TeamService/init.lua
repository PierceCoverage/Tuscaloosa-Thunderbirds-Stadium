...
function TeamService:GetHomeTeam()
	return self._HomeTeam
end

function TeamService:GetAwayTeam()
...
function TeamService.Client:GetHomeTeam(...)
	return self.Server:GetHomeTeam(...)
end

function TeamService.Client:GetAwayTeam(...)
	return self.Server:GetAwayTeam(...)
end
...
function TeamService:KnitStart()
	...
	for _, default_team in pairs(script.Default:GetChildren()) do
		task.spawn(function()
			...
			if team_object.Name == "Away" then
				self._AwayTeam = team
			elseif team_object.Name == "Home" then
				self._HomeTeam = team
			end
		end)
	end
	...
	GameService:Update({
		HomeTeam = Teams:FindFirstChild(self._HomeTeam.TeamData and self._HomeTeam.TeamData.Name or "Home"),
