...
function PingService:Ping(player)
	...
	if
		player.Team == Teams:FindFirstChild(TeamService:GetHomeTeam().Name)
		or player.Team == Teams:FindFirstChild(TeamService:GetAwayTeam().Name)
	then
		if self._PlayerTable[player.Name] then
			if (tick() - self._PlayerTable[player.Name]) > self.MaxPing then
				...
				for i, v in pairs(Teams["Game Officials"]:GetPlayers()) do
					task.spawn(function()
